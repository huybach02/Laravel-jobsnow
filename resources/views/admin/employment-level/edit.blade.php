...
@section('content')
    <div class="block-header border border-bottom">
        <h2 class="d-flex align-items-center"><i class="zmdi zmdi-settings zmdi-hc-spin mr-2" style="font-size: 24px"></i>Quản
            Lý Cấp Bậc Việc Làm</h2>
        <ul class="breadcrumb ml-4">
            <li class="breadcrumb-item"><a href="index.html">Quản lý thông tin ứng viên</a></li>
            <li class="breadcrumb-item">Cấp bậc việc làm</li>
            <li class="breadcrumb-item active">Chỉnh sửa cấp bậc việc làm << <span class="text-primary">
                    {{ $employmentLevel->name }} </span>>>
            </li>
        </ul>
        <a href="{{ route('admin.employment-levels.index') }}">
    ...
    <div>
        <div class="card">
            <div class="header">
                <h2 class="text-primary"> Chỉnh sửa cấp bậc việc làm <span
                        class="text-danger">({{ $employmentLevel->name }})</span>
                </h2>
            </div>
            <div class="body">
                <form action="{{ route('admin.employment-levels.update', $employmentLevel->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="col-12 p-0">
                        <b>Tên cấp bậc việc làm</b>
                        <div class="input-group d-flex flex-column">
                            <div class="form-line {{ $errors->has('name') ? 'error focused' : '' }}">
                                <input type="text" class="form-control" placeholder="VD: Tiếng Việt, Tiếng Anh,..."
                                    name="name" value="{{ old('name', $employmentLevel->name) }}">
                            </div>
                            @if ($errors->has('name'))
                    ...
                    <div class="col-12 p-0">
                        ...
                        <select class="form-control border mt-2" name="status">
                            <option {{ old('status', $employmentLevel->status) == '1' ? 'selected' : '' }} value="1">
                                Kích
                                hoạt</option>
                            <option {{ old('status', $employmentLevel->status) == '0' ? 'selected' : '' }} value="0">
                                Không
                                kích hoạt</option>
                        </select>
                    </div>

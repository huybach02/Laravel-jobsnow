        ...
        <div class="card">
            <div class="header">
                <h2 class="text-primary"> Chỉnh sửa cấp bậc việc làm <span
                        class="text-danger">({{ $experience->name }})</span>
                </h2>
            </div>
            <div class="body">
                <form action="{{ route('admin.experiences.update', $experience->id) }}" method="POST">
                        ...
                        <div class="input-group d-flex flex-column">
                            <div class="form-line {{ $errors->has('name') ? 'error focused' : '' }}">
                                <input type="text" class="form-control" placeholder="VD: Tiếng Việt, Tiếng Anh,..."
                                    name="name" value="{{ old('name', $experience->name) }}">
                            </div>
                            @if ($errors->has('name'))
                    ...
                    <div class="col-12 p-0">
                        ...
                        <select class="form-control border mt-2" name="status">
                            <option {{ old('status', $experience->status) == '1' ? 'selected' : '' }} value="1">
                                Kích
                                hoạt</option>
                            <option {{ old('status', $experience->status) == '0' ? 'selected' : '' }} value="0">
                                Không
                                kích hoạt</option>
                        </select>
                    </div>

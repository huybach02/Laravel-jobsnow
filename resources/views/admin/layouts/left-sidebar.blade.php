<aside id="leftsidebar" class="sidebar">
    ...
    <div class="menu mt-5">
        <ul class="list">
            ...
            @if (canAccess(['Truy cập mục QL Thông Tin Tổ Chức']))
                <li
                    class="{{ setActive(['admin.industry-types.*', 'admin.organization-types.*', 'admin.team-sizes.*']) }}">
            ...
            @if (canAccess(['Truy cập mục QL Địa Điểm']))
                <li class="{{ setActive(['admin.province-cities.*', 'admin.districts.*', 'admin.commune-wards.*']) }}">
                    <a href="javascript:void(0);" class="menu-toggle d-flex align-items-center">
            ...
            @if (canAccess(['Truy cập mục QL TT Ứng Viên']))
                <li
            ...
            @if (canAccess(['Xem danh sách Gói Dịch Vụ']))
                <li class="{{ setActive(['admin.plans.*']) }}"> <a href="{{ route('admin.plans.index') }}"
                        class="d-flex align-items-center"><i class="zmdi zmdi-view-carousel"></i><span
                            class="font-bold">QL
                            Gói Dịch Vụ</span> </a>
            @endif

            @if (canAccess(['Xem danh sách đơn hàng']))
            ...
            @if (canAccess(['Truy cập mục QL TT Tuyển Dụng']))
                <li
            ...
            @if (canAccess(['Truy cập mục QL Tin Tuyển Dụng']))
                <li class="{{ setActive(['admin.posts.*']) }}"> <a href="{{ route('admin.posts.index') }}"
                        class="d-flex align-items-center"><i class="zmdi zmdi-view-list"></i><span
                            class="font-bold">QL
                            Tin Tuyển Dụng</span> </a>
                </li>
            @endif

            @if (canAccess('Truy cập mục QL Tin Tức'))
            ...
            @if (canAccess(['Truy cập mục QL Trang Chủ']))
                <li class="{{ setActive(['admin.home-section.*', 'admin.featured-categories-section.*']) }}">
                    ...
                    <ul class="ml-menu">
                        ...
                        <li class="{{ setSubActive(['admin.featured-categories-section.*']) }}"> <a
                                href="{{ route('admin.featured-categories-section.index') }}">Danh mục nổi bật</a>
                        </li>
            ...
            @if (canAccess(['Truy cập mục QL Người Dùng']))
                <li class="{{ setActive(['admin.roles.*', 'admin.role-user.*']) }}">
                    <a href="javascript:void(0);" class="menu-toggle d-flex align-items-center">
                        <i class="zmdi zmdi-accounts-alt"></i>
                        <span class="font-bold">QL Người Dùng</span>
                    </a>
                    <ul class="ml-menu">
                        <li class="{{ setSubActive(['admin.roles.*']) }}"> <a
                                href="{{ route('admin.roles.index') }}">Vai trò</a></li>
                        <li class="{{ setSubActive(['admin.role-user.*']) }}"> <a
                                href="{{ route('admin.role-user.index') }}">Người dùng</a></li>
                    </ul>
                </li>
            @endif
        </ul>

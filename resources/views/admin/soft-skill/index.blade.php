...
@section('content')
    <div class="block-header border border-bottom">
        ...
        <ul class="breadcrumb ml-4">
            ...
            <li class="breadcrumb-item active">Kỹ năng mềm</li>
        </ul>
    </div>
    <div>
        <div class="card">
            <div class="header d-flex align-items-center justify-content-between">
                <h2 class="text-primary">Danh sách kỹ năng mềm </h2>
                <a href="{{ route('admin.soft-skills.create') }}">
                        ...
                        <tbody>
                            @foreach ($softSkills as $key => $softSkill)
                                <tr>
                                    ...
                                    <td>{{ $softSkill->slug }}</td>
                                    <td>
                                        @if ($softSkill->status == 1)
                                            <div class="switch">
                                                <label>
                                                    <input class="change-status" type="checkbox" checked
                                                        data-id="{{ $softSkill->id }}"
                                                        data-url="{{ route('admin.soft-skills.change-status') }}">
                                    ...
                                    <td>
                                        <a href="{{ route('admin.soft-skills.edit', $softSkill->id) }}">
                                            <button type="button" class="btn btn-raised bg-blue waves-effect btn-xs"><i
                                                    class="zmdi zmdi-edit"></i></button>
                                        </a>

                                        <a href="{{ route('admin.soft-skills.destroy', $softSkill->id) }}"

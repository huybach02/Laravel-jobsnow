    ...
    <div>
        <div class="card">
            <div class="header d-flex align-items-center justify-content-between">
                <h2 class="text-primary">Danh sách loại hình tổ chức </h2>
                        ...
                        <tbody>
                            @foreach ($organizationTypes as $key => $organizationType)
                                <tr>
                                    ...
                                    <td>
                                        @if ($organizationType->status == 1)
                                            <div class="switch">
                                                <label>
                                                    <input class="change-status" type="checkbox" checked
                                                        data-id="{{ $organizationType->id }}"
                                                        data-url="{{ route('admin.organization-types.change-status') }}">
                                    ...
                                    <td>
                                        <a href="{{ route('admin.organization-types.edit', $organizationType->id) }}">
                                            <button type="button" class="btn btn-raised bg-blue waves-effect btn-xs"><i
                                                    class="zmdi zmdi-edit"></i></button>
                                        </a>

                                        <a href="{{ route('admin.organization-types.destroy', $organizationType->id) }}"

...
@section('content')
    <div class="block-header border border-bottom">
        ...
        <ul class="breadcrumb ml-4">
            ...
            <li class="breadcrumb-item">Trình độ học vấn</li>
            <li class="breadcrumb-item active">Chỉnh sửa trình độ học vấn << <span class="text-primary">
                    {{ $academicLevel->name }} </span>>>
            </li>
        </ul>
    ...
    <div>
        <div class="card">
            <div class="header">
                <h2 class="text-primary"> Chỉnh sửa trình độ học vấn <span
                        class="text-danger">({{ $academicLevel->name }})</span>
                </h2>
            </div>
            <div class="body">
                <form action="{{ route('admin.academic-levels.update', $academicLevel->id) }}" method="POST">
                        ...
                        <div class="input-group d-flex flex-column">
                            <div class="form-line {{ $errors->has('name') ? 'error focused' : '' }}">
                                <input type="text" class="form-control" placeholder="VD: Tiếng Việt, Tiếng Anh,..."
                                    name="name" value="{{ old('name', $academicLevel->name) }}">
                            </div>
                            @if ($errors->has('name'))
                    ...
                    <div class="col-12 p-0">
                        ...
                        <select class="form-control border mt-2" name="status">
                            <option {{ old('status', $academicLevel->status) == '1' ? 'selected' : '' }} value="1">
                                Kích
                                hoạt</option>
                            <option {{ old('status', $academicLevel->status) == '0' ? 'selected' : '' }} value="0">
                                Không
                                kích hoạt</option>
                        </select>
                    </div>

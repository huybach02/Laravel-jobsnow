...
@section('content')
    <div class="block-header border border-bottom">
        ...
        <ul class="breadcrumb ml-4">
            ...
            <li class="breadcrumb-item">Hình thức trả lương</li>
            <li class="breadcrumb-item active">Chỉnh sửa hình thức trả lương << <span class="text-primary">
                    {{ $salaryStructure->name }} </span>>>
            </li>
        </ul>
    ...
    <div>
        <div class="card">
            <div class="header">
                <h2 class="text-primary"> Chỉnh sửa hình thức trả lương <span
                        class="text-danger">({{ $salaryStructure->name }})</span></h2>
            </div>
            <div class="body">
                <form action="{{ route('admin.salary-structures.update', $salaryStructure->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="input-group d-flex flex-column">
                            <div class="form-line {{ $errors->has('name') ? 'error focused' : '' }}">
                                <input type="text" class="form-control"
                                    placeholder="VD: Trả lương theo giờ, ngày, tháng,..." name="name"
                                    value="{{ old('name', $salaryStructure->name) }}">
                            </div>
                            @if ($errors->has('name'))
                    ...
                    <div class="col-12 p-0">
                        ...
                        <select class="form-control border mt-2" name="status">
                            <option {{ old('status', $salaryStructure->status) == '1' ? 'selected' : '' }} value="1">
                                Kích
                                hoạt</option>
                            <option {{ old('status', $salaryStructure->status) == '0' ? 'selected' : '' }} value="0">
                                Không
                                kích hoạt</option>
                        </select>
                    </div>

...
@section('content')
    <div class="block-header border border-bottom">
        ...
        <ul class="breadcrumb ml-4">
            ...
            <li class="breadcrumb-item">Lĩnh vực nghề nghiệp</li>
            <li class="breadcrumb-item active">Chỉnh sửa lĩnh vực nghề nghiệp << <span class="text-primary">
        ...
        <div class="card">
            <div class="header">
                <h2 class="text-primary"> Chỉnh sửa lĩnh vực hoạt động <span
                        class="text-danger">({{ $profession->name }})</span></h2>
            </div>
            <div class="body">
                <form action="{{ route('admin.professions.update', $profession->id) }}" method="POST">
                        ...
                        <div class="input-group d-flex flex-column">
                            <div class="form-line {{ $errors->has('name') ? 'error focused' : '' }}">
                                <input type="text" class="form-control" placeholder="VD: Tiếng Việt, Tiếng Anh,..."
                                    name="name" value="{{ old('name', $profession->name) }}">
                            </div>
                            @if ($errors->has('name'))
                    ...
                    <div class="col-12 p-0">
                        ...
                        <select class="form-control border mt-2" name="status">
                            <option {{ old('status', $profession->status) == '1' ? 'selected' : '' }} value="1">Kích
                                hoạt</option>
                            <option {{ old('status', $profession->status) == '0' ? 'selected' : '' }} value="0">Không
                                kích hoạt</option>
                        </select>
                    </div>
